...
```{r 2, echo=FALSE, warning=FALSE, results='hide'}
...
```{r 31,echo=FALSE,warning=FALSE}

print(dfSummary(UniversalBank1,graph.magnif = 0.75, method = 'render'))
...
> This Confusion Matrix has a Accuracy of 0.968 and Specifity of 0.738, lower than the Sensitivity. Kappa is equal to 0.793.
Among all Confusion Matrix, this one has the highest Specifity and Kappa. This Confusion Matrix has the highest F1 but really close to the last Confusion Matrix. 
...
> Specificity is 0.669 which is lower than the Sensitivity (0.99), and kappa of 0.735 with accuracy of 0.96.
...
> The Specificity is higher (0.716) than the previous Confusion Matrix (0.669 - Train VS Valid), but still lower than the Sensitivity. The Accuracy is a bit higher 0.965 > 0.96, but Kappa is higher with this Confusion Matrix (0.767>0.735). 
...
$$\small P (CC=1 | Loan=1) = \\ \small \dfrac{95}{297} = 0.3198653 = 31.99 \%$$
